    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 rounded-card bg-surface border border-positive-line flex items-center gap-4 relative overflow-hidden">
          ...
          <div>
            <div className="text-2xl font-black text-ink font-mono">{completedCount}</div>
            <div className="text-xs font-medium text-ink-muted">Completed Credentials</div>
          </div>
        </div>
        ...
        <div className="p-5 rounded-card bg-surface border border-caution-line flex items-center gap-4 relative overflow-hidden">
          ...
          <div>
            <div className="text-2xl font-black text-ink font-mono">{inProgressCount}</div>
            <div className="text-xs font-medium text-ink-muted">Active Upskilling Pathways</div>
          </div>
        </div>
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
          <div className="flex items-start gap-3">
            <div className="p-3 rounded-card bg-caution-soft text-caution-ink border border-caution-line shrink-0">
      ...
      <div className="p-6 rounded-panel bg-surface border border-line shadow-pop space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-ink uppercase font-mono tracking-wider">
          ...
          {PRESET_PATHWAYS.map((p, idx) => (
            ...
            >
              <div>
                ...
                <h4 className="text-xs font-bold text-ink leading-snug">{p.title}</h4>
              </div>
        ...
        <div className="p-6 rounded-panel bg-surface border border-accent-line shadow-pop space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-line">
            ...
            <button
              ...
              className="text-xs text-ink-muted hover:text-ink p-1 rounded-control cursor-pointer"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <h3 className="text-xs font-bold text-ink-muted uppercase tracking-wider font-mono flex items-center gap-2">
            <span>Active Learning Pathways ({courses.length})</span>
            <span className="w-2 h-2 rounded-full bg-positive animate-pulse" />
          </h3>

          {/* Filter tabs */}
          <div className="flex items-center gap-1.5 p-1 rounded-control bg-fill border border-line">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    >
                      {isDone ? (
                        ...
                        <>
                          <Clock className="w-3.5 h-3.5 text-caution-ink" />
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {course.extracted_skills.map((skill) => (
                        <span
                          key={skill}
                          className={`px-2.5 py-1 text-xs font-mono rounded-control font-bold ${
                            isDone
                ...
                <div className="pt-4 mt-4 border-t border-line flex items-center justify-between text-xs text-ink-muted">
                  <div className="flex items-center gap-1.5 text-xs">
                    {isDone ? (
                      ...
                      <span className="text-caution-ink font-mono flex items-center gap-1">
